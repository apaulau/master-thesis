DrawDataRepresentation <- function(data, filename = "", datebreaks) {
    plot.source <- ggplot(data, aes(x = year, y = temperature)) + geom_point() + geom_line() + scale_x_continuous(breaks = datebreaks) +
...
# plot is based on some magic; investigate again what is this magic is // TODO: look up can I introduce new variable with
# y-breaks
DrawScatterPlot <- function(data, filename = "", datebreaks) {
    plot.scatter <- ggplot(data, aes(x = year, y = temperature)) + geom_point() + geom_smooth(method = "lm", se = FALSE,
        fullrange = TRUE) + scale_x_continuous(breaks = datebreaks, limits = c(min(datebreaks), max(datebreaks))) + scale_y_continuous(breaks = seq(10,
        30, 1), limits = c(14, 26)) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + xlab("Год наблюдения") +
        ylab("Температура, ºС")
...
DrawHistogram <- function(data, filename = "", binwidth = 1.2, fit = TRUE) {
    plot.hist <- ggplot(data, aes(x = temperature), geom = "blank") + geom_histogram(aes(y = ..density..), colour = "darkgrey",
        fill = "white", binwidth = binwidth, alpha = 0.6) + labs(color = "") + xlab("Температура, ºС") + ylab("Гистограмма")

    if (fit) {
        plot.hist <- plot.hist + stat_function(fun = dnorm, colour = "#D55E00", args = list(mean = mean(data$temperature),
            sd = sd(data$temperature)))
    }
...
DrawTimeSeries <- function(data, filename = "", datebreaks) {
    plot.ts <- ggplot(data, aes(x = year, y = temperature)) + geom_point() + geom_line() + stat_smooth(method = lm, se = FALSE) +
        scale_x_continuous(breaks = datebreaks) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + xlab("Год наблюдения") +
        ylab("Температура, ºС")
...
DrawHScatterplot <- function(data) {
    ...
    hsc <- hscat(data ~ 1, p, breaks = dist.breaks)
...
DrawParameterComparison <- function(cutoffs, classical, robust, adapt) {
    ggplot() + geom_line(data = data.frame(X = cutoffs, Y = classical), aes(x = X, y = Y, linetype = "Матерона")) +
        ...
        scale_x_continuous(breaks = cutoffs) + xlab("Максимальное расстояние") + ylab(ifelse(adapt,
        "MSE", "Корреляция")) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
}
