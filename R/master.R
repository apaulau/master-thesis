...
print(
  xtable(sample.dstats, caption = "Описательные статистики для наблюдаемых температур.", label = "table:dstats"),
  caption.placement = 'top', file = "out/original/dstats.tex"
)
...
WriteTest(
  sample.ctest$statistic, sample.ctest$p.value, sample.ctest$parameter[[1]], type = "original", name = "correlation"
)
...
print(
  xtable(
    tmp, caption = "Временной ряд остатков", label = "table:residuals", digits = c(0, 0, 2), align = "r|rc|"
...
print(
  xtable(
    sample.residuals.dstats, caption = "Описательные статистики остатков", label = "table:residuals_dstats"
...
WriteTest(
  sample.residuals.box$statistic, sample.residuals.box$p.value, sample.residuals.box$parameter[[1]], type = "residual", name = "ljung-box"
)

sample.residuals.adf <- adf.test(sample.residuals$temperature)
WriteTest(
  sample.residuals.adf$statistic, sample.residuals.adf$p.value, type = "residual", name = "stationarity"
)
