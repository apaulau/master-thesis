autoKrige = function(formula, input_data, new_data, data_variogram = input_data, block = 0,
                     model = c("Nug", "Exp", "Sph", "Gau", "Exc", "Mat", "Ste", "Cir", "Lin", "Bes", "Pen", "Per", 
                               "Wav", "Hol", "Log", "Pow", "Spl", "Leg", "Err", "Int"), kappa = c(0.05, seq(0.2, 2, 0.1), 5, 10), 
                     fix.values = c(NA,NA,NA), remove_duplicates = TRUE, verbose = FALSE, GLS.model = NA,
                     start_vals = c(NA,NA,NA), miscFitOptions = list(), len, ...)
...
autoKrige1 = function(formula, input_data, new_data, data_variogram = input_data, block = 0,
                     model = c("Nug", "Exp", "Sph", "Gau", "Exc", "Mat", "Ste", "Cir", "Lin", "Bes", "Pen", "Per", 
                               "Wav", "Hol", "Log", "Pow", "Spl", "Leg", "Err", "Int"), kappa = c(0.05, seq(0.2, 2, 0.1), 5, 10), 
                     fix.values = c(NA,NA,NA), remove_duplicates = TRUE, verbose = FALSE, GLS.model = NA,
                     start_vals = c(NA,NA,NA), miscFitOptions = list(), len, ...)
  # This function performs an automatic Kriging on the data in input_data
{
  ...
  # Fit the variogram model, first check which model is used
  variogram_object = autofitVariogram1(formula,
                                      data_variogram,
                                      #autoselect.model = autoselect.model,
                                      model = model,
                                      kappa = kappa,
